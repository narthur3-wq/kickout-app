<script>
  ...
  function remove(i){ draft = draft.filter((_,idx)=>idx!==i); }
    ...
    <div class="space-y-2 max-h-[50vh] overflow-auto">
      {#each draft as p, i}
        <div class="flex gap-2">
          <input class="flex-1 h-9 rounded-md border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-900 px-2"
                 bind:value={draft[i]} placeholder={`Player ${i+1}`} />
          <button class="h-9 px-3 rounded-md border border-rose-300 text-rose-600 dark:border-rose-700" on:click={()=>remove(i)}>Remove</button>
        </div>
      {/each}
