<script>
  ...
  $: zoneMap = (() => {
    ...
    for (const r of $filtered) {
      ...
      const m = map[z] || (map[z] = { total:0, retained:0, breaks:0, breaks_won:0 });
      m.total += 1;
      if (r.outcome === 'retained') m.retained += 1;
      if (r.contest_type === 'break') { m.breaks += 1; if (r.break_outcome === 'won') m.breaks_won += 1; }
    }
    return map;
  })();
...
<style>
  ...
  table{ width:100%; border-collapse:collapse } th,td{ padding:.35rem .5rem; border-bottom:1px solid #eee; text-align:left }
